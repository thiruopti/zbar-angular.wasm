export default interface ZBar extends Record<string, WebAssembly.ExportValue> {
    ...
    ImageScanner_set_config(scanner: number, symbology: number, config: number, value: number): number;
    ...
    Image_create(width: number, height: number, format: number, data: number, dataLength: number, sequenceNum: number): number;
